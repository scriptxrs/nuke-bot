...
# Embeds, Utility, Forward, Forward2, Forward3, Clone,
# Auto-Protection, Admin Giver, Ultra-Fast Webhook Spam
# ============================================================
...
import discord
from discord import Intents, Embed, Color, File, Member, TextChannel, app_commands, ui
from discord.ext import commands
from discord.ui import Button, View, Select, Modal, TextInput, ButtonStyle
...
async def fetch_guild_data(guild_id, token):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.get(f"https://discord.com/api/v10/guilds/{guild_id}/members?limit=1000", headers=headers) as resp:
            members = await resp.json() if resp.status == 200 else []
        return guild, channels, roles, members
...
async def download_website_real(base_url, max_pages=50):
    """Download a website and return ZIP data"""
    visited = set()
    to_visit = [base_url]
    files = {}
    file_count = 0
...
async def help_cmd(interaction: discord.Interaction):
    ...
    commands_list = [
        ...
        ("🧹 Cleanup", "/clean, /purge, /clear, /deletetext, /deletefiles"),
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(f"https://discord.com/api/v10/channels/{source_channel_id}", headers=headers) as resp:
                if resp.status == 403:
                    await interaction.followup.send("❌ Permission Denied! Can't view that channel.", ephemeral=True)
                    return
        ...
        file_count = 0
        await interaction.followup.send(f"📤 Found {len(messages)} messages, forwarding files...", ephemeral=True)
            ...
            for att in attachments:
                try:
                    ...
                    if file_size > 25 * 1024 * 1024:
                        await target_channel.send(f"⚠️ File too large: {filename} ({file_size/1024/1024:.1f}MB)")
                        continue
                    ...
                    await target_channel.send(f"⚠️ Failed: {str(e)[:50]}")
...
@app_commands.default_permissions(administrator=True)
@app_commands.describe(
    source_channel_id="Channel ID to scan for loadstrings",
    target_channel="Channel to send loadstrings to",
...
async def forward_loadstrings_cmd(
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(f"https://discord.com/api/v10/channels/{source_channel_id}", headers=headers) as resp:
                if resp.status != 200:
                    await interaction.followup.send("❌ Cannot access channel!", ephemeral=True)
                    return
                ...
                if matches:
                    for match in matches:
                        loadstrings.append({
                            ...
                            'author_id': msg['author']['id'],
                            'timestamp': msg.get('timestamp', '')[:19],
                            'loadstring': match if isinstance(match, str) else str(match),
                        })
...
@app_commands.describe(
    ...
    target_channel="Channel to send the ZIP to",
...
async def clone_cmd(
    ...
    await interaction.response.send_message(f"📋 **Cloning...**\nSource: `{source_id}`\nTarget: `{target_id}`", ephemeral=True)
    
    try:
        guild_data, channels_data, roles_data, members_data = await fetch_guild_data(source_id, DISCORD_TOKEN)
        ...
        # Delete all channels
        await interaction.edit_original_response(content="🗑️ Clearing target server...")
        ...
        # Clone roles
        await interaction.edit_original_response(content="📋 Cloning roles...")
        role_map = {}
        ...
        # Clone categories
        await interaction.edit_original_response(content="📂 Creating categories...")
        cat_map = {}
        ...
        # Clone text channels
        await interaction.edit_original_response(content="💬 Creating text channels...")
        ...
        await interaction.edit_original_response(content="🔊 Creating voice channels...")
...
@app_commands.default_permissions(administrator=True)
@app_commands.describe(user="User to give admin (default: yourself)")
...
async def spam_webhook_cmd(
    ...
    await interaction.followup.send(f"🌐 **SPAM STARTED!**\nCount: {count}\nRunning in background...", ephemeral=True)
...
@app_commands.describe(
    ...
    message="Message to spam",
    count_per_webhook="Messages per webhook"
)
async def spam_multi_cmd(
    interaction: discord.Interaction,
    webhook_urls: str,
    message: str,
    count_per_webhook: int = 100
):
    await interaction.response.defer(ephemeral=True)
    ...
    await interaction.followup.send(f"🌐 **MULTI SPAM STARTED!**\nWebhooks: {len(urls)}\nTotal: {len(urls) * count_per_webhook}", ephemeral=True)
    
    tasks = []
    for url in urls:
        tasks.append(spam_webhook_task_quick(url, message, count_per_webhook))
    ...
    await interaction.followup.send(f"✅ **MULTI SPAM COMPLETE!**\n📤 Sent: {total_sent}\n❌ Failed: {total_failed}", ephemeral=True)
...
@app_commands.describe(user="User to timeout", minutes="Minutes", reason="Reason")
...
@app_commands.describe(user="User", nickname="New nickname (empty to reset)")
...
@app_commands.describe(seconds="Seconds between messages", channel="Channel")
async def slowmode_cmd(interaction: discord.Interaction, seconds: int, channel: TextChannel = None):
...
async def lock_cmd(interaction: discord.Interaction, channel: TextChannel, reason: str = "No reason"):
    await interaction.response.defer(ephemeral=True)
...
async def unlock_cmd(interaction: discord.Interaction, channel: TextChannel):
    await interaction.response.defer(ephemeral=True)
...
async def clear_cmd(interaction: discord.Interaction, channel: TextChannel = None):
    target = channel or interaction.channel
...
async def deletetext_cmd(interaction: discord.Interaction, channel: TextChannel = None, limit: int = 1000):
    target = channel or interaction.channel
...
@app_commands.default_permissions(manage_messages=True)
@app_commands.describe(channel="Channel", limit="Max messages")
async def deletefiles_cmd(interaction: discord.Interaction, channel: TextChannel = None, limit: int = 1000):
    target = channel or interaction.channel
...
async def ticketsetup_cmd(interaction: discord.Interaction, channel: TextChannel):
...
async def embed_cmd(interaction: discord.Interaction, channel: TextChannel, title: str, description: str, color: str = None, footer: str = None):
...
async def announce_cmd(interaction: discord.Interaction, channel: TextChannel, title: str, message: str):
...
async def poll_cmd(interaction: discord.Interaction, channel: TextChannel, question: str, option1: str, option2: str, option3: str = None, option4: str = None):
...
async def say_cmd(interaction: discord.Interaction, message: str, channel: TextChannel = None):
    target = channel or interaction.channel
...
@bot.tree.command(name="clearwarnings", description="🗑️ Clear all warnings")
@app_commands.default_permissions(moderate_members=True)
